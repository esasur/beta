import {NavParams, NavController, ToastController, ModalController, ViewController, Events} from 'ionic-angular';
...
class modalites {
  ...
  constructor(private viewCtrl: ViewController, private mikiPublicite: MikiPubliciteService, private params: NavParams) {
...
export class EventDetails3 {
  ...
  constructor(private toastCtrl: ToastController, private modalCtrl: ModalController, private navParams: NavParams, private events: MikiEventsService, private mikiPublicite: MikiPubliciteService, private http: Http, private e: Events, private nav: NavController) {
  ...
  initForm(){
    ...
    if (!this.user || this.user.country == ''){
      for(let i = 0; i < this.countries.length; i++) {
        if(this.countries[i].name.fr == 'Suisse'){
          this.eventSubscribeForm.controls.subscriptionPrivateCountry.updateValue(this.countries[i].id);
        }
      }
    }
    ...
    if (!this.user || this.user.company.country == ''){
      for(let i = 0; i < this.countries.length; i++) {
        if(this.countries[i].name.fr == 'Suisse'){
          this.eventSubscribeForm.controls.subscriptionProCountry.updateValue(this.countries[i].id);
        }
      }
    }
  }
  ...
  openConditions(){
    ...
    this.http.get('http://es-asur.ch/api/index.php/page/' + pageName).subscribe(
      data => {
