import {App, NavController, ionicBootstrap, Platform, MenuController, AlertController, Events, Storage, LocalStorage} from 'ionic-angular';
...
@Component({
  ...
  // providers: [MikiPersonService, MikiPubliciteService],
...
export class MyApp {
  ...
  constructor(private app: App, private platform: Platform, private menu: MenuController, private events: Events, private mikiPerson: MikiPersonService, private mikiPublicite: MikiPubliciteService, public alertCtrl: AlertController) {
    ...
    platform.ready().then(() => {
      ...
      this.mikiPublicite.refresh().then(data => {
        ...
        this.nav.setRoot(PubliciteHome, { pubType: 'start', user: this.mikiPerson.user });
      });
      ...
      this.events.subscribe('homePublicite:closed', (pubType) => {
        if (pubType == 'start'){
          ...
          this.nav.setRoot(EventsList, { idCategory: 1, user: this.mikiPerson.user });
        }
        else if (pubType == 'ps_home'){
          this.nav.setRoot(EventsList, { idCategory: 15, user: this.mikiPerson.user });
        }
      });
  ...
  openEvents(idCategory?: number){
    ...
    if (this.rootPage != this.EventsList){  
      ...
      if (idCategory != undefined){
        ...
        // this.nav.setRoot(EventsList, { idCategory: idCategory, user: this.mikiPerson.user });
      }
      else{
        this.nav.setRoot(EventsList, { user: this.mikiPerson.user });
      }
    }
...
ionicBootstrap(MyApp, [MikiPersonService, MikiPubliciteService], {
